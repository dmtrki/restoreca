...
export default {
  ...
  computed: {
    ...
    isParted() {
      if (this.$route.name !== 'product') return false
      if (this.cartItems.find(it => it.uuid === this.$route.params.uuid)) return false
      return true
    },
    isCartActive() {
      return (this.$route.name === 'cart') ? true : false
    },
    isCartDisabled() {
