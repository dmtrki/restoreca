<template>
  <nuxt-link :to="to" :class="classList" :style="style">
...
export default {
  ...
  computed: {
    classList() {
      let classes = 'featured'
      if (this.modifiers) {
        this.modifiers.forEach(element => {
          classes += ' featured--'+element
        });
      }
      return classes
    },
    style() {
      ...
      if (this.backgroundGradient) {
        let bg = {
              direction: this.backgroundGradient.direction ?? '145.24deg',
              bg_start: this.backgroundGradient.direction ?? '#4E019C',
              startPos: this.backgroundGradient.direction ?? '5.64%',
              bg_finish: this.backgroundGradient.direction ?? '#0098B9',
              finishPos: this.backgroundGradient.direction ?? '92.31%'
            },
